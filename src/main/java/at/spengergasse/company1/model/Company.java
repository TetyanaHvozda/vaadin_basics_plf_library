...
public class Company implements Serializable {
    ...
    public static void loadFromFile(InputStream inputStream) throws CompanyException {
        companyInstance = Company.readFromFile( inputStream );
    }
...
//            }
    ...
    public Employee getEmployee(Long id) throws CompanyException {
        return staff.stream().filter(e -> e.getId().equals(id))
                    .findFirst().orElseThrow( () -> new CompanyException("Error: no employee with id " + id + " exists. " ));
                    // .orElseThrow(new CompanyException("Error: no employee with id " + id + " exists. " ));
    }
    ...
    public int numberOfRole(Employee.Role role) {
        return (int)(staff.stream().filter(e -> e.getRole().equals(role)).count());
    }
    ...
    public static Company readFromFile(InputStream stream) throws CompanyException {
        if(stream == null)
            throw new CompanyException("Stream must not be null!");
        ...
        try (BufferedReader br = new BufferedReader( new InputStreamReader( stream ) )) {
            ...
            throw new CompanyException("Error", e);
        }
